...
```
This design provides a `Account` class with methods to meet all specified requirements for managing an account in a trading simulation platform. Each method raises appropriate exceptions to handle invalid operations, ensuring robustness in user interactions. The transactions are recorded for auditing, and the portfolio value is calculated dynamically based on current stock prices.